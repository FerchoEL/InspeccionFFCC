                ...
                <table class="table table-striped ">
                    ...
                    @foreach ($users as $user)
                        <tr>
                            ...
                            <td>{{$user->getRoleNames()->first() !== null ? $user->getRoleNames()->first() : 'No tiene Rol'}}</td>
                            <td>{{$user->active == 1 ? 'Activo' : 'Desactivo'}}</td>
                            <td width='10px'>
                                @if($user->active == 1)
                                    {!! Form::model($user,['route'=> ['menu.users.desactivate',$user], 'method' => 'POST']) !!}
                                    {!! Form::submit('Desactivar', ['class' => 'btn btn-danger']) !!}
                                    {!! Form::close() !!} 
                                @else
                                    {!! Form::model($user,['route'=> ['menu.users.activate',$user], 'method' => 'POST']) !!}
